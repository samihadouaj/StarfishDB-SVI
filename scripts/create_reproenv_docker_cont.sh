...
DOCKER_CONT_NAME="starfishdb_repro_env_container_${ORIG_USER}${PROJECT_ROOT_ABS_PATH_HASH}"

docker run --name ${DOCKER_CONT_NAME} --replace --detach --tty ${DOCKER_IMG_NAME} /bin/bash
